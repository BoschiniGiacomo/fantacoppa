...
app.use('/api', (req, res, next) => {
  ...
  const currentVersionCode = readClientVersionCode(req);

  if (isBelowMinSupported(currentVersionCode)) {
    // Caso eccezione (< 17): niente 426 (crash). Soft update su /public/app-loading.
    if (isCrashyForceUpdateClient(currentVersionCode)) {
      return next();
    }

    return res.status(426).json(buildForceUpdatePayload(currentVersionCode));
  }
