...
const { getSoftForceUpdateFields } = require('../utils/appVersionGate');

/**
 * GET /api/public/app-loading
 * Pubblico (nessun token): usato all'avvio app prima del login.
 * Se la versione client è sotto la min, include update_required (soft force-update,
 * anche per client < 17 che non devono ricevere HTTP 426).
 */
router.get('/app-loading', async (_req, res) => {
  try {
    ...
    const softUpdate = getSoftForceUpdateFields(_req);
    return res.json({
      path: pathVal || null,
      type: typeVal,
      ...(softUpdate || {}),
