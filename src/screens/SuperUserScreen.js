...
function buildUnclusteredPlayersList(players, groupMeta) {
  return (players || []).map((player) => {
    const groupId = Number(player.group_id != null ? player.group_id : groupMeta?.group_id);
    const groupName = player.group_name || groupMeta?.group_name || '';
...
export default function SuperUserScreen() {
  ...
  const [openingOfficialGroupId, setOpeningOfficialGroupId] = useState(null);
  ...
  const clusterFilterBtnRef = useRef(null);
  ...
  const [neverPlayedInfoOpen, setNeverPlayedInfoOpen] = useState(false);
  ...
  const [clusterFilterMenuLayout, setClusterFilterMenuLayout] = useState(null);
  ...
  const [clusterYearFilterOpen, setClusterYearFilterOpen] = useState({
    league: false,
    birth: false,
  });
  ...
  const unclusteredHydratedRef = useRef(false);
  const officialGroupsHydratedRef = useRef(false);
  ...
  // Carica giocatori singoli solo quando il filtro è attivo (1 HTTP, non per gruppo)
  useEffect(() => {
    if (!isSuperuser || !clusterFilters.includeSingles) return undefined;
    if (unclusteredHydratedRef.current) return undefined;
    let cancelled = false;
    (async () => {
      try {
        ...
        unclusteredHydratedRef.current = true;
      } catch (error) {
        if (cancelled) return;
  ...
  const loadOfficialGroups = async ({ silent = false } = {}) => {
    if (!isSuperuser) return;
    try {
      if (!silent || officialGroups.length === 0) setLoadingOfficialGroups(true);
      const response = await superuserService.getOfficialGroups();
      setOfficialGroups(Array.isArray(response.data) ? response.data : []);
      setOfficialGroupsDisabled(false);
      officialGroupsHydratedRef.current = true;
    } catch (error) {
  ...
  const fetchAllApprovedClusters = async () => {
    const response = await superuserService.getPlayerClustersOverview('approved');
    return Array.isArray(response.data?.clusters) ? response.data.clusters : [];
  };

  const fetchAllUnclusteredPlayers = async () => {
    const res = await superuserService.getAllUnclusteredPlayers();
    const players = res.data?.players || [];
    return buildUnclusteredPlayersList(players).sort((a, b) => {
      const nameCmp = a.name.localeCompare(b.name, 'it');
      if (nameCmp !== 0) return nameCmp;
  ...
  const loadApprovedClustersByPlayer = async ({ includeSingles = false, silent = false } = {}) => {
    if (!isSuperuser) return;
    try {
      if (!silent || approvedClustersByPlayer.length === 0) setLoadingApprovedClusters(true);
      if (includeSingles) {
      ...
      clustersOverviewHydratedRef.current = true;
    } catch (error) {
      if (isFeatureDisabledError(error)) {
      ...
      showToast(clusterApiErrorMessage(error, 'Impossibile caricare i cluster approvati'));
    } finally {
  ...
  const refreshSelectedPlayerClusterAfterChange = async (clusterId) => {
    const needSingles = unclusteredHydratedRef.current || clusterFilters.includeSingles;
    const [allClusters, allSingles] = await Promise.all([
      fetchAllApprovedClusters(),
      needSingles ? fetchAllUnclusteredPlayers() : Promise.resolve(null),
    ]);
    const updatedList = buildApprovedClustersList(allClusters);
    setApprovedClustersByPlayer(updatedList);
    if (Array.isArray(allSingles)) {
      setUnclusteredPlayersByPlayer(allSingles);
      unclusteredHydratedRef.current = true;
    }
  ...
  const toggleGroupLeagueExpanded = (leagueId) => {
    setExpandedGroupLeagueIds((prev) => {
      const opening = !prev[leagueId];
      if (opening) {
        const league = (selectedGroupForEdit?.leagues || []).find((l) => Number(l.id) === Number(leagueId));
        const groupId = Number(selectedGroupForEdit?.id);
        if (
          Number(league?.official_two_groups) === 1
          && groupId > 0
          && !Object.prototype.hasOwnProperty.call(gironiTeamsByLeague, leagueId)
        ) {
          void (async () => {
            try {
              const gt = await superuserService.getOfficialLeagueGironiTeams(groupId, leagueId);
              setGironiTeamsByLeague((map) => ({ ...map, [leagueId]: gt.data?.teams || [] }));
            } catch (_) {
              setGironiTeamsByLeague((map) => ({ ...map, [leagueId]: [] }));
            }
          })();
        }
      }
      return { ...prev, [leagueId]: opening };
    });
  };
  ...
  useEffect(() => {
    if (!showClusterFilters) {
      setClusterFilterMenuLayout(null);
      setClusterYearFilterOpen({ league: false, birth: false });
      return undefined;
    }
    let cancelled = false;
    const measureAnchor = () => {
      const node = clusterFilterBtnRef?.current;
      if (!node || typeof node.measureInWindow !== 'function') return;
      try {
        node.measureInWindow((x, y, width, height) => {
          if (cancelled) return;
  ...
  }, [showClusterFilters, windowWidth]);
  ...
  const closeClusterFilters = () => {
    setShowClusterFilters(false);
  };
  ...
  const renderClusterFilterCheckRow = (key, label, on, onPress, last) => (
    <TouchableOpacity
      key={key}
      style={[
        styles.userFilterDropdownItem,
        on && styles.userFilterDropdownItemOn,
        last && styles.userFilterDropdownItemLast,
      ]}
      onPress={onPress}
      activeOpacity={0.8}
    >
  ...
  const renderClusterFilterCollapseHeader = (key, label, open, summary) => (
    <TouchableOpacity
      key={key}
      style={styles.userFilterCollapseHeader}
      onPress={() => setClusterYearFilterOpen((prev) => ({ ...prev, [key]: !prev[key] }))}
      activeOpacity={0.75}
    >
      <Text style={styles.userFilterCollapseLabel}>{label}</Text>
      <View style={styles.userFilterCollapseRight}>
        {summary ? (
          <Text style={styles.userFilterCollapseSummary} numberOfLines={1}>
            {summary}
          </Text>
        ) : null}
        <Ionicons
  ...
  const renderClusterItem = ({ item }) => {
    const title = formatClusterListTitle(item.name, item.leagues);
    const initial = String(title || '?').trim().charAt(0).toUpperCase() || '?';
    const editions = item.is_single_player
      ? 1
      : Number(item.players_count || item.leagues?.length || 0);
    const subtitle = [
      `${editions} ${editions === 1 ? 'edizione' : 'edizioni'}`,
      item.group_name || null,
    ].filter(Boolean).join(' · ');
    return (
      <TouchableOpacity
        style={styles.clusterRow}
        onPress={() => openPlayerCluster(item)}
        activeOpacity={0.72}
      >
        <View style={[styles.clusterAvatar, item.is_single_player && styles.clusterAvatarSingle]}>
        ...
        <View style={styles.playerClusterInfo}>
          <View style={styles.playerClusterNameRow}>
            <Text style={styles.clusterRowName} numberOfLines={1}>
              {title}
            </Text>
          ...
          <Text style={styles.clusterRowSub} numberOfLines={1}>{subtitle}</Text>
        </View>
  ...
  const renderClustersSkeleton = () => (
    <View style={styles.usersSkeletonWrap}>
      {Array.from({ length: 8 }).map((_, idx) => (
        <View key={`csk-${idx}`} style={styles.userRowSkeleton}>
          <View style={styles.userAvatarSkeleton} />
          <View style={styles.userSkeletonLines}>
            <View style={[styles.userSkeletonBar, { width: `${54 + (idx % 3) * 12}%` }]} />
  ...
  const seedGironiTeamsFromLeagues = (leagues) => {
    const next = {};
    for (const lg of leagues || []) {
      if (Array.isArray(lg.gironi_teams)) next[lg.id] = lg.gironi_teams;
    }
    if (Object.keys(next).length) {
      setGironiTeamsByLeague((prev) => ({ ...prev, ...next }));
    }
  };
  ...
  const openOfficialGroup = async (item) => {
    ...
    try {
      ...
      seedGironiTeamsFromLeagues(leaguesInGroup);
      setShowGroupDetailModal(true);
    } catch (error) {
      console.error('Error loading group leagues:', error);
      showToast('Impossibile caricare le leghe del gruppo');
    } finally {
      setOpeningOfficialGroupId(null);
    }
  };

  const renderOfficialGroupItem = ({ item }) => {
    const leagueCount = Number(item.league_count || 0);
    const inMenu = Number(item.show_in_main_menu) === 1;
    const initial = String(item.name || '?').trim().charAt(0).toUpperCase() || '?';
    const logoUri = item.logo_path ? publicAssetUrl(item.logo_path) : null;
    const opening = openingOfficialGroupId === item.id;
    const subtitle = [
    ...
    return (
      <TouchableOpacity
        style={styles.officialGroupRow}
        onPress={() => openOfficialGroup(item)}
        activeOpacity={0.72}
        disabled={!!openingOfficialGroupId}
      >
        {logoUri ? (
          <Image source={{ uri: logoUri }} style={styles.officialGroupAvatarImg} />
        ) : (
          <View style={[styles.officialGroupAvatar, inMenu && styles.officialGroupAvatarOn]}>
            <Text style={styles.officialGroupAvatarText}>{initial}</Text>
          </View>
        )}
        <View style={styles.officialGroupInfo}>
          <View style={styles.officialGroupNameRow}>
            <Text style={styles.officialGroupName} numberOfLines={1}>{item.name}</Text>
            {inMenu ? (
              <View style={styles.officialGroupMenuBadge}>
                <Text style={styles.officialGroupMenuBadgeText}>Menu</Text>
              </View>
            ) : null}
          </View>
          <Text style={styles.officialGroupStats} numberOfLines={1}>{subtitle}</Text>
        </View>
        {opening ? (
  ...
  const renderOfficialsSkeleton = () => (
    <View style={styles.usersSkeletonWrap}>
      {Array.from({ length: 5 }).map((_, idx) => (
        <View key={`osk-${idx}`} style={styles.userRowSkeleton}>
          <View style={styles.userAvatarSkeleton} />
          <View style={styles.userSkeletonLines}>
            <View style={[styles.userSkeletonBar, { width: `${48 + (idx % 3) * 14}%` }]} />
            <View style={[styles.userSkeletonBar, styles.userSkeletonBarShort]} />
          </View>
        </View>
      ))}
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        >
          <Ionicons 
            name={activeTab === 'clusters' ? "git-network" : "git-network-outline"} 
            size={20} 
      ...
      <View style={styles.content}>
            ...
            {loadingOfficialGroups && officialGroups.length === 0 ? (
              ...
              <View style={styles.officialsTabBody}>
                <FlatList
                  ...
                  renderItem={renderOfficialGroupItem}
                  initialNumToRender={12}
                  maxToRenderPerBatch={12}
                  windowSize={7}
                  refreshControl={
                    <RefreshControl
                      refreshing={refreshingOfficialGroups}
                      tintColor="#667eea"
                      colors={['#667eea']}
                      onRefresh={() => {
                        setRefreshingOfficialGroups(true);
                        officialGroupsHydratedRef.current = false;
                        void loadOfficialGroups({ silent: true });
                      }}
                    />
                  }
                  ListEmptyComponent={
                  ...
                  contentContainerStyle={styles.usersListContent}
                />
                <View style={[styles.officialsTools, { paddingBottom: 12 + Math.max(insets.bottom, 8) }]}>
                  <TouchableOpacity
                    style={styles.officialsToolRow}
                    onPress={openClusterYearGapsModal}
                    activeOpacity={0.75}
                  >
                    <View style={styles.officialsToolIcon}>
                      <Ionicons name="git-commit-outline" size={18} color="#667eea" />
                    </View>
                    <Text style={styles.officialsToolTitle}>Buchi anni cluster</Text>
                    <Ionicons name="chevron-forward" size={18} color="#cbd5e1" />
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.officialsToolRow}
                  ...
                  >
                    <View style={styles.officialsToolIcon}>
                      <Ionicons name="build-outline" size={18} color="#667eea" />
                    </View>
                    <Text style={styles.officialsToolTitle}>Ripara discrepanze</Text>
                    <Ionicons name="chevron-forward" size={18} color="#cbd5e1" />
                  </TouchableOpacity>
                </View>
              </View>
        ...
        {activeTab === 'clusters' && (
          <>
            <View style={styles.usersSearchRow}>
              <View style={[styles.usersSearchContainer, styles.usersSearchContainerFlex]}>
                ...
                {clusterTabSearchText.length > 0 && (
                  <TouchableOpacity onPress={() => setClusterTabSearchText('')} style={styles.clearButton} hitSlop={8}>
                    <Ionicons name="close-circle" size={18} color="#94a3b8" />
                  </TouchableOpacity>
                )}
              </View>
              <TouchableOpacity
                ...
                style={[styles.usersFilterBtn, hasActiveClusterFilters && styles.usersFilterBtnActive]}
              ...
              >
                ...
                <View style={[
                  styles.usersFilterCountBadge,
                  hasActiveClusterFilters && styles.usersFilterCountBadgeActive,
                ]}
                >
                  <Text
                    style={[
                      styles.usersFilterCountBadgeText,
                      hasActiveClusterFilters && styles.usersFilterCountBadgeTextActive,
                    ]}
                    numberOfLines={1}
                  >
            ...
            {loadingApprovedClusters && approvedClustersByPlayer.length === 0 ? (
                ...
                refreshControl={
                  <RefreshControl
                    ...
                    onRefresh={() => {
                      ...
                      clustersOverviewHydratedRef.current = false;
                      unclusteredHydratedRef.current = false;
            ...
            <Modal
              ...
              transparent
              animationType="fade"
              onRequestClose={closeClusterFilters}
            >
              <View style={styles.userFilterMenuRoot}>
                <Pressable
                  style={styles.userFilterMenuBackdrop}
                  onPress={closeClusterFilters}
                  accessibilityRole="button"
                  accessibilityLabel="Chiudi filtri cluster"
                />
                {clusterFilterMenuLayout ? (
                  <View
                    style={[
                      styles.userFilterDropdown,
                      {
                        top: clusterFilterMenuLayout.top,
                        left: clusterFilterMenuLayout.left,
                        width: clusterFilterMenuLayout.width,
                        maxHeight: 460,
                      },
                    ]}
                  >
                    <View style={styles.userFilterDropdownHeader}>
                      ...
                      <TouchableOpacity
                        style={[
                          styles.userFilterPresetChip,
                          !hasActiveClusterFilters && styles.userFilterPresetChipActive,
                        ]}
                        onPress={clearClusterFilters}
                        activeOpacity={0.75}
                      >
                        <Text
                          style={[
                            styles.userFilterPresetChipText,
                            !hasActiveClusterFilters && styles.userFilterPresetChipTextActive,
                          ]}
                        >
                          Azzera
                        </Text>
                    ...
                    <ScrollView
                      style={[styles.userFilterDropdownScroll, { maxHeight: 410 }]}
                      contentContainerStyle={styles.userFilterDropdownScrollContent}
                      showsVerticalScrollIndicator={false}
                      keyboardShouldPersistTaps="handled"
                      bounces={false}
                      nestedScrollEnabled
                    >
                      <Text style={styles.userFilterSectionLabel}>Gruppo ufficiale</Text>
                      {clusterFilterOptions.groups.length > 0 ? (
                        clusterFilterOptions.groups.map((group, idx, arr) => (
                          renderClusterFilterCheckRow(
                            `g-${group.id}`,
                            group.name,
                            clusterFilters.groupId === group.id,
                            () => toggleClusterFilter('groupId', group.id),
                            idx === arr.length - 1,
                          )
                        ))
                      ) : (
                        <Text style={styles.userFilterEmptyHint}>Nessun gruppo</Text>
                      )}

                      {renderClusterFilterCollapseHeader(
                        'league',
                        'Anno lega',
                        clusterYearFilterOpen.league,
                        (clusterFilters.leagueYears || []).length
                          ? (clusterFilters.leagueYears || []).join(', ')
                          : null,
                      )}
                      {clusterYearFilterOpen.league ? (
                        clusterFilterOptions.leagueYears.length > 0 ? (
                          clusterFilterOptions.leagueYears.map((year, idx, arr) => (
                            renderClusterFilterCheckRow(
                              `ly-${year}`,
                              String(year),
                              (clusterFilters.leagueYears || []).includes(year),
                              () => toggleClusterFilterArray('leagueYears', year),
                              idx === arr.length - 1,
                            )
                          ))
                        ) : (
                          <Text style={styles.userFilterEmptyHint}>Nessun anno</Text>
                        )
                      ) : null}
                      ...
                      {renderClusterFilterCollapseHeader(
                        'birth',
                        'Anno di nascita',
                        clusterYearFilterOpen.birth,
                        (clusterFilters.birthYears || []).length
                          ? (clusterFilters.birthYears || [])
                            .map((y) => (y === BIRTH_YEAR_FILTER_NONE ? 'Nessuno' : (formatClusterBirthYearShort(y) || String(y))))
                            .join(', ')
                          : null,
                      )}
                      {clusterYearFilterOpen.birth ? (
                        <>
                          {renderClusterFilterCheckRow(
                            'by-none',
                            'Nessuno',
                            (clusterFilters.birthYears || []).includes(BIRTH_YEAR_FILTER_NONE),
                            () => toggleClusterFilterArray('birthYears', BIRTH_YEAR_FILTER_NONE),
                            clusterFilterOptions.birthYears.length === 0,
                          )}
                          {clusterFilterOptions.birthYears.map((year, idx, arr) => (
                            renderClusterFilterCheckRow(
                              `by-${year}`,
                              formatClusterBirthYearShort(year) || String(year),
                              (clusterFilters.birthYears || []).includes(year),
                              () => toggleClusterFilterArray('birthYears', year),
                              idx === arr.length - 1,
                            )
                          ))}
                        </>
                      ...
                      <Text style={styles.userFilterSectionLabel}>Giocatori singoli</Text>
                      {renderClusterFilterCheckRow(
                        'singles',
                        'Mostra singoli',
                        !!clusterFilters.includeSingles,
          ...
          >
            <View style={styles.userDetailSheetHeader}>
              ...
              <View style={styles.userDetailSheetHeaderRow}>
                <Text style={styles.userDetailSheetTitle}>Gruppo ufficiale</Text>
                <TouchableOpacity style={styles.userDetailCloseBtn} onPress={closeGroupDetailModal} hitSlop={8}>
                  <Ionicons name="close" size={20} color="#64748b" />
                </TouchableOpacity>
              </View>
              <View style={styles.groupDetailHeaderActionsRow}>
                <TouchableOpacity
                  style={styles.groupDetailHeaderAction}
                  onPress={() => void handleOpenGroupClusters()}
                >
                  <View style={styles.groupDetailHeaderIconBtn}>
                    <Ionicons name="people-outline" size={20} color="#667eea" />
                  </View>
                  <Text style={styles.groupDetailHeaderActionLabel}>Cluster</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.groupDetailHeaderAction}
                ...
                >
                  ...
                  <Text style={styles.groupDetailHeaderActionLabel}>Mai giocato</Text>
                </TouchableOpacity>
                {selectedGroupForEdit ? (
                  ...
                  >
                    ...
                    <Text
                      style={[
                        styles.groupDetailHeaderActionLabel,
                        Number(selectedGroupForEdit.show_in_main_menu) === 1 &&
                          styles.groupDetailHeaderActionLabelOn,
                      ]}
                    >
                      Menu
                    </Text>
            ...
            {selectedGroupForEdit ? (
              ...
              >
                <View style={styles.groupProfileHero}>
                  ...
                  <Text style={styles.groupProfileName} numberOfLines={2}>
                    {selectedGroupForEdit.name || 'Gruppo ufficiale'}
                  </Text>
                  {selectedGroupForEdit.description ? (
                    <Text style={styles.groupProfileDescription} numberOfLines={3}>
                      {selectedGroupForEdit.description}
                    </Text>
                  ) : null}
                  {Number(selectedGroupForEdit.show_in_main_menu) === 1 ? (
                    <View style={[styles.officialGroupMenuBadge, { marginTop: 8 }]}>
                ...
                {selectedGroupForEdit.leagues && selectedGroupForEdit.leagues.length > 0 ? (
                  selectedGroupForEdit.leagues.map((league) => {
                    ...
                    const squadPublic = Number(league.is_official_squad_public || 0) === 1;
                    const twoGroups = Number(league.official_two_groups || 0) === 1;
                    const gironiReady = Object.prototype.hasOwnProperty.call(
                      gironiTeamsByLeague,
                      league.id
                    );
                    const gironiTeams = gironiTeamsByLeague[league.id] || [];
                    return (
                      <View key={league.id} style={styles.groupLeagueItem}>
                        ...
                        >
                          <View style={styles.groupLeagueHeaderText}>
                            ...
                            <Text style={styles.groupLeagueSummary} numberOfLines={1}>
                              {getLeagueDisplayYear(league)} · {formatCreationDate(league.created_at)}
                            </Text>
                          </View>
                          <Ionicons
                          ...
                          <View style={styles.groupLeagueExpandedBody}>
                            <View style={styles.groupLeagueReferenceYearRow}>
                              ...
                              >
                                ...
                                <Text style={styles.groupLeagueReferenceYearPickerBtnText}>
                                  {String(referenceYearDrafts[String(league.id)] ?? '').trim() || 'Anno'}
                                </Text>
                              </TouchableOpacity>
                              <TouchableOpacity
                                style={styles.groupLeagueReferenceYearClearBtn}
                                onPress={() =>
                                  setReferenceYearDrafts((prev) => ({
                                    ...prev,
                                    [String(league.id)]: '',
                                  }))
                                }
                            ...
                            <View style={styles.groupLeagueSettingRow}>
                              <View style={styles.groupLeagueSettingCopy}>
                                <Text style={styles.groupLeagueSettingTitle}>Rosa pubblica</Text>
                                <Text style={styles.groupLeagueSettingSub}>
                                  {squadPublic
                                    ? 'Visibile negli anni della squadra ufficiale'
                                    : 'Bozza: nascosta dai selettori'}
                                </Text>
                              </View>
                              <Switch
                                value={squadPublic}
                                ...
                                thumbColor={squadPublic ? '#667eea' : '#f4f3f4'}
                              />
                            </View>

                            <View style={styles.groupLeagueSettingRow}>
                              <View style={styles.groupLeagueSettingCopy}>
                                <Text style={styles.groupLeagueSettingTitle}>Due gironi</Text>
                                <Text style={styles.groupLeagueSettingSub}>
                                  Partite Gironi solo nello stesso girone
                                </Text>
                              </View>
                              <Switch
                                value={twoGroups}
                                onValueChange={(v) =>
                                  handleToggleTwoOfficialGroups(selectedGroupForEdit.id, league, v)
                                }
                                trackColor={{ false: '#e2e8f0', true: '#a5b4fc' }}
                                thumbColor={twoGroups ? '#667eea' : '#f4f3f4'}
                              />
                            </View>

                            {twoGroups ? (
                              <View style={styles.groupLeagueGironiList}>
                                {!gironiReady ? (
                                  <Text style={styles.groupLeagueGironiEmpty}>Caricamento squadre…</Text>
                                ) : gironiTeams.length === 0 ? (
                                  <Text style={styles.groupLeagueGironiEmpty}>Nessuna squadra</Text>
                                ) : (
                                  gironiTeams.map((tm) => (
                                    <View
                                      key={`gir-${league.id}-${tm.id}`}
                                      style={styles.groupLeagueGironiRow}
                                    >
                                      <Text
                                        style={styles.groupLeagueGironiName}
                                        numberOfLines={1}
                                      >
                                        {tm.name}
                                      </Text>
                                      <View style={styles.groupLeagueGironiChips}>
                                        <TouchableOpacity
                                          onPress={() =>
                                            handleAssignTeamGirone(
                                          ...
                                          style={[
                                            styles.groupLeagueGironiChip,
                                            Number(tm.girone_index) === 1 &&
                                              styles.groupLeagueGironiChipOn,
                                          ]}
                                        >
                                          <Text
                                            style={[
                                              styles.groupLeagueGironiChipText,
                                              Number(tm.girone_index) === 1 &&
                                                styles.groupLeagueGironiChipTextOn,
                                            ]}
                                          >
                                            A
                                          </Text>
                  ...
                  <Text style={styles.groupDetailEmpty}>Nessuna lega in questo gruppo</Text>
                )}

                <TouchableOpacity
                  style={styles.groupDeleteRow}
                  onPress={() => {
                    setConfirmModal({
                ...
                >
                  ...
                  <Text style={styles.groupDeleteRowText}>Elimina gruppo</Text>
                </TouchableOpacity>
              </ScrollView>
            ) : null}
          </View>
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            <View style={styles.userDetailSheetHeader}>
              ...
              <View style={styles.userDetailSheetHeaderRow}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    neverPlayedInfoOpen && styles.discrepancyInfoIconBtnActive,
                  ]}
                  onPress={() => setNeverPlayedInfoOpen((v) => !v)}
                  hitSlop={8}
                  accessibilityLabel="Info su mai giocato"
                >
                  <Ionicons
                    name="information-circle-outline"
                    size={22}
                    color={neverPlayedInfoOpen ? '#667eea' : '#64748b'}
                  />
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.userDetailCloseBtn}
              ...
              {neverPlayedInfoOpen ? (
                <View style={styles.neverPlayedInfoBannerWrap}>
                  <View style={styles.discrepancyInfoBanner}>
                    <Text style={styles.discrepancyInfoBannerText}>
                      ...
                      {'\n'}
                      Contano come “mai giocato”: nessun voto oppure solo N.D. — esclusi chi ha
                      almeno un S.V. o un voto reale.
                    </Text>
                  </View>
                </View>
              ) : null}
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            <View style={styles.userDetailSheetHeader}>
              ...
              <View style={styles.userDetailSheetHeaderRow}>
                <View style={{ flex: 1, paddingRight: 8 }}>
                  <Text style={styles.userDetailSheetTitle}>Buchi anni cluster</Text>
                </View>
                <TouchableOpacity
                  style={[
                    styles.discrepancyInfoIconBtn,
                ...
                >
                  <Ionicons
                    name="information-circle-outline"
                    size={22}
                    color={yearGapsInfoOpen ? '#667eea' : '#64748b'}
                  />
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.userDetailCloseBtn}
                  onPress={closeClusterYearGapsModal}
                  hitSlop={8}
                >
                  <Ionicons name="close" size={20} color="#64748b" />
                </TouchableOpacity>
            ...
            <Text style={[styles.clusterDetailSectionTitle, styles.toolsSheetSectionTitle]}>
              Gruppo ufficiale
            </Text>
            <ScrollView
              horizontal
            ...
            <View style={styles.toolsSheetActionWrap}>
              <TouchableOpacity
                style={[
                  styles.officialsToolRow,
                  (!yearGapsGroupId || loadingClusterYearGaps) && styles.toolsSheetActionDisabled,
                ]}
              ...
              >
                ...
                <Text style={styles.officialsToolTitle}>
            ...
            {clusterYearGapsResult ? (
                  ...
                  <>
                    ...
                    >
                      <TouchableOpacity
                        style={[
                          styles.usersSortChip,
                          yearGapsYearFilter == null && styles.usersSortChipActive,
                        ]}
                        onPress={() => {
                          setYearGapsYearFilter(null);
                          setYearGapsFillTarget(null);
                      ...
                      >
                        <Text
                          style={[
                            styles.usersSortChipText,
                            yearGapsYearFilter == null && styles.usersSortChipTextActive,
                          ]}
                        >
                          Tutti
                        </Text>
                            ...
                            <View style={styles.yearGapsFillPanel}>
                              ...
                              {(yearGapsFillTarget.gap.existing_players || []).length > 0 ? (
                                <View style={styles.yearGapsExistingBlock}>
                                  <Text style={styles.clusterDetailSectionTitle}>Già in rosa</Text>
                                  {yearGapsFillTarget.gap.existing_players.map((ep) => (
                                    <TouchableOpacity
                                      key={`yg-ex-${ep.player_id}`}
                                      style={[
                                        styles.yearGapsExistingBtn,
                                        savingYearGapsFill && styles.toolsSheetActionDisabled,
                                      ]}
                              ...
                              <Text style={styles.clusterDetailSectionTitle}>Ruolo</Text>
                                  ...
                                  return (
                                    <TouchableOpacity
                                      ...
                                      style={[
                                        ...
                                        r === 'P' && active && { backgroundColor: '#dbeafe', borderColor: '#93c5fd' },
                                        r === 'D' && active && { backgroundColor: '#dcfce7', borderColor: '#86efac' },
                                        r === 'C' && active && { backgroundColor: '#fef3c7', borderColor: '#fde68a' },
                                        r === 'A' && active && { backgroundColor: '#fee2e2', borderColor: '#fecaca' },
                                      ]}
                                    ...
                                    >
                                      <Text
                                        style={[
                                          ...
                                          r === 'P' && active && { color: '#1d4ed8' },
                                          r === 'D' && active && { color: '#15803d' },
                                          r === 'C' && active && { color: '#a16207' },
                                          r === 'A' && active && { color: '#b91c1c' },
                                        ]}
                                      >
                                        {r}
                                      </Text>
                              ...
                              <Text style={styles.clusterDetailSectionTitle}>Squadra</Text>
                              <ScrollView
                                horizontal
                                showsHorizontalScrollIndicator={false}
                                  ...
                                  return (
                                    <TouchableOpacity
                                      ...
                                      style={[
                                        ...
                                        active && styles.usersSortChipActive,
                                      ]}
                              ...
                              <TouchableOpacity
                                style={[
                                  styles.clusterCreateRow,
                                  (savingYearGapsFill || !yearGapsFillTeamId) && styles.toolsSheetActionDisabled,
                                ]}
                              ...
                              >
                                ...
                                <Text style={styles.officialsToolTitle}>Crea e aggiungi al cluster</Text>
                              </TouchableOpacity>
                            </View>
                          ) : null}
                        </View>
              ...
              <View style={styles.neverPlayedEmpty}>
                <Ionicons name="git-commit-outline" size={40} color="#cbd5e1" />
                <Text style={styles.neverPlayedEmptyTitle}>Seleziona un gruppo e avvia la ricerca</Text>
                <Text style={styles.neverPlayedEmptySub}>
                  Verranno analizzati i cluster con anni mancanti tra il primo e l’ultimo presente.
                </Text>
              </View>
            ) : null}
          </View>
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            <View style={styles.userDetailSheetHeader}>
              ...
              <View style={styles.userDetailSheetHeaderRow}>
                <View style={{ flex: 1, paddingRight: 8 }}>
                  <Text style={styles.userDetailSheetTitle}>Ripara discrepanze</Text>
                </View>
                <TouchableOpacity
                  style={[
                    styles.discrepancyInfoIconBtn,
                ...
                >
                  <Ionicons
                    name="information-circle-outline"
                    size={22}
                    color={discrepancyInfoOpen ? '#667eea' : '#64748b'}
                  />
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.userDetailCloseBtn}
                  onPress={closeLiveBonusDiscrepancyModal}
                  hitSlop={8}
                >
                  <Ionicons name="close" size={20} color="#64748b" />
                </TouchableOpacity>
            ...
            <View style={styles.toolsSheetActionWrap}>
              <TouchableOpacity
                style={[
                  styles.officialsToolRow,
                  (!discrepancyGroupId || loadingLiveBonusDiscrepancies) && styles.toolsSheetActionDisabled,
                ]}
            ...
            {liveBonusDiscrepancyResult ? (
              <>
                <View style={styles.toolsSheetSummaryCard}>
                  <Text style={styles.toolsSheetSummaryText}>
                    {liveBonusDiscrepancyResult.match_count || 0} partite ·{' '}
                    {liveBonusDiscrepancyResult.player_count || 0} giocatori ·{' '}
                    {liveBonusDiscrepancyResult.discrepancy_count || 0} differenze
                  </Text>
                  <Text style={styles.toolsSheetSummarySub}>
                ...
                <View style={styles.clusterModalTabs}>
                  <TouchableOpacity
                    style={[
                      styles.usersSortChip,
                      discrepancyViewMode === 'matches' && styles.usersSortChipActive,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.usersSortChipText,
                        discrepancyViewMode === 'matches' && styles.usersSortChipTextActive,
                      ]}
                    >
                      Per partita
                    </Text>
                  ...
                  <TouchableOpacity
                    style={[
                      styles.usersSortChip,
                      discrepancyViewMode === 'players' && styles.usersSortChipActive,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.usersSortChipText,
                        discrepancyViewMode === 'players' && styles.usersSortChipTextActive,
                      ]}
                    >
                      Per giocatore
                    </Text>
                ...
                <View style={styles.usersSearchContainer}>
                  ...
                  {discrepancySearchText.length > 0 ? (
                    <TouchableOpacity
                      onPress={() => setDiscrepancySearchText('')}
                      style={styles.clearButton}
                    >
                      <Ionicons name="close-circle" size={18} color="#94a3b8" />
                    </TouchableOpacity>
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            <View style={styles.userDetailSheetHeader}>
              ...
              <View style={styles.userDetailSheetHeaderRow}>
                <View style={{ flex: 1, paddingRight: 8 }}>
                  <Text style={styles.userDetailSheetTitle} numberOfLines={1}>
            ...
            {selectedPlayerCluster ? (() => {
              ...
              return (
              <View style={styles.clusterMetaCard}>
            ...
            {selectedPlayerCluster && (
              <ScrollView
                ...
                showsVerticalScrollIndicator={false}
              >
                <View style={styles.clusterDetailSection}>
                  <Text style={styles.clusterDetailSectionTitle}>
                    Edizioni ({selectedPlayerCluster.leagues.length})
                  </Text>
                  {selectedPlayerCluster.leagues.length > 0 ? (
                    selectedPlayerCluster.leagues.map((league, index) => {
                      ...
                      return (
                        <View key={`${league.cluster_id}-${league.player_id}-${index}`} style={styles.clusterEditionRow}>
                          <View style={styles.clusterLeagueColLeague}>
                            <Text style={styles.clusterEditionLeague} numberOfLines={1}>{league.name}</Text>
                            {league.group_name ? (
                              <Text style={styles.clusterEditionGroup} numberOfLines={1}>
                                {league.group_name}
                              </Text>
                            ) : null}
                          </View>
                          <View style={styles.clusterLeagueColTeam}>
                            <Text style={styles.clusterEditionTeam} numberOfLines={2}>
                              {league.team_name || '—'}
                            </Text>
                            <Text style={styles.clusterEditionRole}>
...
const styles = StyleSheet.create({
  ...
  officialGroupAvatarOn: {
    backgroundColor: '#e0e7ff',
  },
  officialGroupAvatarImg: {
    width: 40,
    height: 40,
    borderRadius: 20,
    backgroundColor: '#f1f5f9',
  },
  officialGroupAvatarText: {
    fontSize: 16,
    fontWeight: '800',
    color: '#667eea',
  },
  ...
  toolsSheetSectionTitle: {
    paddingHorizontal: 16,
    marginTop: 8,
    marginBottom: 6,
  },
  toolsSheetActionWrap: {
    paddingHorizontal: 16,
    marginTop: 4,
    marginBottom: 8,
  },
  toolsSheetActionDisabled: {
    opacity: 0.5,
  },
  toolsSheetSummaryCard: {
    marginHorizontal: 16,
    marginBottom: 8,
    backgroundColor: '#fff',
    borderRadius: 12,
  ...
  toolsSheetSummaryText: {
    fontSize: 13,
    fontWeight: '700',
    color: '#0f172a',
  },
  toolsSheetSummarySub: {
    fontSize: 11,
    color: '#64748b',
    marginTop: 2,
  },
  ...
  groupDetailHeaderAction: {
    alignItems: 'center',
    minWidth: 64,
  },
  groupDetailHeaderActionLabel: {
    marginTop: 4,
    fontSize: 11,
    fontWeight: '700',
    color: '#94a3b8',
  },
  groupDetailHeaderActionLabelOn: {
    color: '#667eea',
  },
  ...
  groupLeagueSettingTitle: {
    fontSize: 13,
    fontWeight: '700',
    color: '#0f172a',
  },
  groupLeagueSettingSub: {
    marginTop: 2,
    fontSize: 11,
    color: '#64748b',
    lineHeight: 15,
  },
  groupLeagueGironiList: {
    marginTop: 8,
    paddingTop: 8,
    borderTopWidth: 1,
    borderTopColor: '#ececec',
  },
  groupLeagueGironiEmpty: {
    fontSize: 12,
    color: '#94a3b8',
  },
  groupLeagueGironiRow: {
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 8,
  },
  groupLeagueGironiName: {
    flex: 1,
    fontSize: 13,
    color: '#0f172a',
    marginRight: 8,
  },
  groupLeagueGironiChips: {
    flexDirection: 'row',
    gap: 6,
  },
  groupLeagueGironiChip: {
    minWidth: 32,
    paddingHorizontal: 10,
    paddingVertical: 6,
    borderRadius: 8,
  ...
  groupLeagueGironiChipOn: {
    borderColor: '#667eea',
    backgroundColor: '#eef2ff',
  },
  groupLeagueGironiChipText: {
    fontSize: 12,
    fontWeight: '700',
    color: '#64748b',
  },
  groupLeagueGironiChipTextOn: {
    color: '#667eea',
  },
  groupDeleteRow: {
    flexDirection: 'row',
