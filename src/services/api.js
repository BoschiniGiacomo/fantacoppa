...
export const leagueService = {
  ...
  getInviteActive: (leagueId) => api.get(`/leagues/${leagueId}/invites/active`),
  createInvite: (leagueId) => api.post(`/leagues/${leagueId}/invites`),
  revokeInvite: (leagueId) => api.delete(`/leagues/${leagueId}/invites`),
  redeemInvite: (token) => api.post('/leagues/invites/redeem', { token }),
...
export const adminMatchesService = {
  ...
  updateEvent: (matchId, eventId, payload) => api.put(`admin/matches/${matchId}/events/${eventId}`, payload),
  deleteEvent: (matchId, eventId) => api.delete(`admin/matches/${matchId}/events/${eventId}`),
...
export const superuserService = {
  ...
  setOfficialGroupMainMenu: (groupId, showInMainMenu) =>
  ...
  getClusterYearGaps: (groupId) =>
    api.get(`/superuser/official-groups/${encodeURIComponent(Number(groupId))}/cluster-year-gaps`, { timeout: 90000 }),
  fillClusterYearGap: (clusterId, data) =>
    api.post(`/superuser/player-clusters/${encodeURIComponent(Number(clusterId))}/fill-year-gap`, data),
  ...
  uploadLoginBackground: async (formData) => {
    const headers = await buildAuthVersionHeaders();
    return axios.post(`${API_BASE_URL}/superuser/login-background`, formData, {
      headers: {
        ...headers,
        'Content-Type': 'multipart/form-data',
      },
